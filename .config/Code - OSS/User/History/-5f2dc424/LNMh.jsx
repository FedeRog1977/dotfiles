...
export default function HillMarker({ type }) {
    var markers = {};

    if (type === "Munro") {
        markers = hillData.munros.map(({ name, lat, lon, elevation, summit, image }: any) => (
        ...
        markers = hillData.corbetts.map(({ name, lat, lon, elevation, summit, image }: any) => (
            <Marker key={name} position={[lat, lon]}>
                <Popup>
                    <div style={{ textAlign: "center" }}>
                        <h3>{name}</h3>
